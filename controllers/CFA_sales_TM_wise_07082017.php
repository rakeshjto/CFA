...
class CFA_sales_TM_wise extends CI_Controller {
	...
	public function prov_details($designation,$hr_no,$prov_type)
	{
		//$this->output->enable_profiler(TRUE);
		$this->load->model('CFA_sales_TM_wise_model', 'btj');
		$result = $this->btj->prov_details($designation,$hr_no,$prov_type);
	...
	public function custom_reports($designation)
	{
		
		//$this->output->enable_profiler(TRUE);
		$this->load->model('CFA_sales_TM_wise_model', 'btj');
		$result = $this->btj->custom_reports($designation);
		$data = array(
				'title' => 'BEST TM & BEST JTO',
